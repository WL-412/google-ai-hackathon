...
const customCursor = chrome.runtime.getURL('./public/highlight-pen.png');
// Function to update the cursor style based on the mode
function updateCursorStyle() {
  if (highlightModeActive) {
    document.body.style.cursor = `url("${customCursor}"), auto`; // Highlight mode cursor
  } else if (isAnswerMode) {
    document.body.style.cursor = `url("${customCursor}"), auto`; // Answer mode cursor
  } else {
    document.body.style.cursor = ''; // Default cursor
  }
}
