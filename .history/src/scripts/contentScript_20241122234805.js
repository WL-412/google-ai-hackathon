...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.action === "extract_content") {
    ...
    startHighlightMode(message.idx);
...
function startHighlightMode(idx) {
  highlightModeActive = true;
  document.addEventListener('mouseup', handleHighlight(idx));
...
function handleHighlight(index) {
  ...
  if (selection && selection.toString().trim()) {
    ...
    chrome.runtime.sendMessage({ action: "text_highlighted", text: selectedText, index: idx });
    stopHighlightMode();
  }
}